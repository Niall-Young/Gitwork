...
`gitwork` 是实际项目上下文中所有落盘实现任务的强制 Git 收尾门禁。无论技术栈、项目类型、规模、现有内容或 Git 状态，只要任务预计会在桌面端本地项目、CLI 会话目录或 IDE 工作区内创建、修改、重命名或删除文件，即使用户没有提到 Git 或提交也应触发。空文件夹和非 Git 文件夹同样可以是项目；前端、后端、CLI、库、脚本、demo 和原型都只是非穷举示例。

桌面端独立聊天虽然也有文件夹，但其自动工作区位于 `~/Documents/Codex/YYYY-MM-DD/<聊天名称>`。这种目录默认不触发 `gitwork`；只有用户显式调用 `$gitwork` 或明确要求对该聊天目录执行 Git 操作时才加入流程。它会：

- 按当前使用界面确认运行时项目根目录：桌面端使用本地项目主文件夹，CLI 或 IDE 使用会话目录或已打开工作区，不猜测其他路径。
...
它不会用于纯问答、规划、评审、没有文件改动的任务，或默认的桌面端独立聊天目录。它也不会自动推送、改写历史、跳过 Git hooks，或把无关改动混入提交。
...
1. 确认当前使用界面的运行时项目根目录，排除默认独立聊天目录，并检查 Git 操作状态。
